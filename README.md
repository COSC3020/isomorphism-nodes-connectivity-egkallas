...
Consider the edge between distinct nodes $u$ and $v$ in graph $A$. Because $A$ is completely connected, there must be an edge between them. And since there exists bijectivity between graph $A$ and $B$, $(f(u), f(v))$ must exist in graph $B$, where $f$ is the bijective function that maps each node in graph $A$ to each node in graph $B$. This goes for every vertex in both graphs.<br><br>
